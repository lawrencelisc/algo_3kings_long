...
TP_ATR_MULT = 3.5                                       # 止盈 ATR 倍數 🚀 做多版：穩健止盈 (3.5 ATR)，讓升浪利潤奔跑
SL_ATR_MULT = 1.0                                       # 初始止損 ATR 倍數 🚀 做多版：適度止損 (1.0 ATR)，給升勢足夠呼吸空間
# TRAIL_ATR_MULT = 1.0                                  # 追蹤止損 ATR 步進倍數
MIN_IMBALANCE_RATIO = 0.2                               # 訂單簿失衡度門檻 (買盤需厚於賣盤 15%)
...
def get_btc_regime():
    ...
    try:
        ...
        # ⚠️ 做多版 (Long)：HMA20 > HMA50 代表短期動能強於長期，看好升勢
...
def check_flow_health(symbol):
    ...
    try:
        ...
        # ✅ 修正 1：偵測極端狂暴賣盤 (Z-Score < -3.0) 才是多軍的威脅！
        if z_score < -3.0:
            return "Flow Reversal (Long Dump Detected)"

        # ✅ 修正 2：動能衰退預判 (原本升緊，突然有大戶瘋狂賣出)
...
def apply_lee_ready_long_logic(symbol):
    ...
    try:
        ...
        # 🛡️ 防護網 4：防假突破 (Fake Breakout) - 若升得急但賣盤極厚，取消做多
    ...
    try:
        ...
        for p in live_symbols:
            ...
            if side in ['long', 'buy'] or info_side in ['buy', 'long']:
                ...
                # 🚀 [修正] 判斷多單 SL 係咪已經高過入場價 (且大於0)，防止 Trail SL 倒退
...
def manage_long_positions():
    ...
    try:
        ...
        for s in list(positions.keys()):
            ...
            # 階段一 & 二：爬升期推保本
            # ✅ 修正：做多保本必須高於入場價 (1.002) 才能鎖住 0.2% 獲利！
            ...
            if pos['is_breakeven']:
                if pnl_pct > (coin_volatility_pct * 3.5):
                    trail_sl = curr_p - (1.5 * pos['atr'])  # ✅ 修正：減號 (下方)
                else:
                    trail_sl = curr_p - (2.0 * pos['atr'])  # ✅ 修正：減號 (下方)

                # ✅ 修正：多軍止損只准向上移 (新止損 > 舊止損)
            ...
            # ==========================================
            if not exit_reason:
                if curr_p >= pos['tp_price']:  # ✅ 修正：升到或升穿止盈價 (>=)
                    exit_reason = "TP (Long IOC Exit)"
                elif curr_p <= pos['sl_price']:  # ✅ 修正：跌到或跌穿止損價 (<=)
...
def execute_live_long(symbol, net_flow, current_price, is_strong, atr, is_volatile):
    ...
    try:
        ...
        cooldown_tracker[symbol] = time.time() + 480  # 8 分鐘冷卻
